...
public interface HTTPRequestProcessor {
	 ...
	 * 
	 * The response MUST NOT be sent, but filled, because the format the response should be sent depends
	 * on the underlying protocol (which may be at least HTTP/1 or HTTP/2).<br/>
	 * 
	 * The returned synchronization point must be unlocked once the server can start sending the response.
	 ...
	 * code 500 (Server internal error) will be sent to the client.<br/>
	 * 
	 * In case an error occured during the process, the synchronization point can be unlocked with an exception.
	 * If this exception is an HTTPResponseError, the status code it contains will be sent to the client.
	 * Any other exception will result into a status code 500.
	 * 
